...
public class CompanyCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd,
        ...
        if (args.length == 0) { CompanyDashboardGui.open(player, plugin); return true; }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "create" -> {
                ...
                if (!plugin.getEconomy().has(player, cost)) {
                    player.sendMessage(p(plugin.getConfig().getString("messages.insufficient-funds","&cInsufficient funds.")));
                    return true;
                }
                ...
                player.sendMessage(p(plugin.getConfig().getString("messages.company-create-success",
                    "&aCompany &6{name} &aregistered!").replace("{name}", name).replace("{cost}", String.format("%.2f", cost))));
            ...
            case "ipo" -> {
                ...
                player.sendMessage(p(plugin.getConfig().getString("messages.ipo-success",
                    "&aCompany &6{name} &ais now publicly listed!").replace("{name}", c.getName())));
    ...
    private Company requireCompany(Player p) {
        ...
        if (c == null) p.sendMessage(p(plugin.getConfig().getString("messages.no-company","&cNo company.")));
        return c;
    }

    private boolean vaultCheck(Player p) {
        if (plugin.getEconomy() == null) {
            p.sendMessage(p(plugin.getConfig().getString("messages.no-vault","&cVault economy not available.")));
            return false;
        }
        return true;
    }
